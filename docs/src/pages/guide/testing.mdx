...
You can test if the error field contains any message at all, this is useful if you rely on error messages from 3rd party, like the `@zaalbarxx/vee-validate-i18n` package which changes its messages frequently due to constant updates and fixes by the awesome contributors from all over the world.
...
import { FormContextKey, FieldContextKey } from '@zaalbarxx/vee-validate';
